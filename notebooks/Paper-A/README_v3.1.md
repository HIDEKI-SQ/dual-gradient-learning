...
- **Preprint DOI**: [10.5281/zenodo.18215925](https://doi.org/10.5281/zenodo.18215925)
...
| Hysteresis gap | 22–53% depending on method and sweep speed |
| Metastability | 100% survival at λ=0.54 for 300 epochs |
| Trap state (T-state) | All escape interventions failed |
| Causal intervention | Up to 67% performance control via gradient geometry (mean 45%) |
...
| **Tα, Tβ** | 28 | Speed dependence | Gap 22–53% (method/speed-dependent) |
...
| **W2** | 3 | Causal intervention | Gap up to 67% (mean 45%) after 30 epochs |
...
**Speed/method dependence**: R1 dynamic sweep: gap 22% (hold=5 ep/λ) to 45% (hold=1 ep/λ). Tα two-branch protocol: gap up to 53% (3 ep/λ). Gap magnitude depends on both sweep speed and experimental protocol (dynamic vs two-branch).
...
|------------|---|-------:|---------:|------------:|
| **T1a** | 0.46 | 150 | **49/49† (100%)** | 10.3% ± 0.3% |
| **T13** | 0.54 | 300 | **49/49† (100%)** | 9.9% ± 0.3% |

†Of 50 seeds, 49 successfully initialized into L-branch. 1 seed (seed=49) failed initialization (init_error=35.0%, classified as H-branch from start). All 49 seeds that entered L-branch survived for the full duration.
...
| Target λ | L-branch | H-branch | Gap (mean ± SD) | Gap (max) |
|----------|--------:|---------:|:----------------:|----------:|
| 0.40 | 12.8% | 57.6% | 44.8% ± 16.3% | 67.2% |
| 0.50 | 12.7% | 57.4% | 44.7% ± 15.9% | 66.3% |
| 0.55 | 12.9% | 57.0% | 44.1% ± 14.8% | 64.4% |

Note: Large inter-seed variance (3 seeds). Seed 0 shows collapse_init=79.0% while seeds 1,2 show 48.6%/58.7%. Maximum gap (67%) reflects single-seed best case; mean gap (~45%) is the robust estimate.
...
|------------|-------------------|
| R1 | Dynamic sweep: λ = 0.20 → 0.65 → 0.20; R1a hold=1 ep/λ, R1b hold=5 ep/λ |
| R2 | Same as R1 but η=0.8 |
...
| Tα | Two-branch sweep: 1, 3 ep/λ (ordered + collapse branches independently) |
| Tβ1 | Two-branch sweep: 10 ep/λ |
...
|----------|------------|--------|
| **Hysteresis** | R1, Tα | Gap 22–53% between L-branch and H-branch |
